import { generateToken } from "@/js/google_translate_token.js";
...
async function translateText(text, sourceLang, targetLang) {
  // const url = `https://translate.googleapis.com/translate_a/single?client=gtx&sl=${sourceLang}&tl=${targetLang}&dt=t&dt=bd&dj=1&q=${encodeURIComponent(text)}`;
    ...
    var token = await generateToken(text)
    var url = `https://translate.google.com/translate_a/single?client=gtx&sl=${sourceLang}&tl=${targetLang}&hl=${targetLang}&dt=at&dt=bd&dt=ex&dt=ld&dt=md&dt=qca&dt=rw&dt=rm&dt=ss&dt=t&ie=UTF-8&oe=UTF-8&otf=1&ssel=0&tsel=0&kc=7&tk=${token.value}`; // i think the last one is extra here, lol (&TypeError=)
    // tk token, q query, override?
